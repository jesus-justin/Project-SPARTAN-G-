...
import { submitGad7, getGad7Questions } from '../api/assessment.api';
...
export default function Gad7Page() {
  ...
  useEffect(() => {
    async function load() {
      try {
        const res = await getGad7Questions();
        if (res?.data) {
  ...
  const handleSubmit = async () => {
    setSubmitting(true);
    try {
      const res = await submitGad7(answers);
      if (res?.data) {
        setResult({
  ...
  if (result) {
    return (
      <div style={{ maxWidth: 980, margin: '24px auto', padding: 16 }}>
        ...
        <h2 style={{ marginBottom: 20 }}>GAD-7 Results</h2>
      ...
      <div style={{ background: '#fff', padding: 20, borderRadius: 8, border: '1px solid #ddd', marginBottom: 20 }}>
        <div style={{ marginBottom: 20 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}>
            <span style={{ fontWeight: 600 }}>Question {currentIndex + 1} of 7</span>
            <span style={{ color: '#999' }}>Answered: {answeredCount} / 7</span>
          </div>
