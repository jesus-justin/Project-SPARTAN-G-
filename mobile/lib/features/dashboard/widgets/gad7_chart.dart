...
class Gad7Chart extends StatelessWidget {
  const Gad7Chart({super.key, required this.history, required this.description});
  ...
  Widget build(BuildContext context) {
    if (history.isEmpty) {
      return Column(mainAxisSize: MainAxisSize.min, children: <Widget>[
        ...
        const Text('No GAD-7 data yet', style: TextStyle(fontSize: 16)),
        const SizedBox(height: 8),
        ElevatedButton(onPressed: () => Navigator.of(context).pushNamed('/gad7'), child: const Text('Take GAD-7')),
      ]);
    }
    ...
    return Column(
      ...
      children: <Widget>[
        ...
        const SizedBox(height: 8),
        LinearProgressIndicator(value: latestScore / 21.0, color: _severityColor(latestScore), backgroundColor: Colors.grey.shade200, minHeight: 10),
        const SizedBox(height: 8),
        Row(children: <Widget>[Text('$latestScore / 21', style: const TextStyle(fontSize: 20, fontWeight: FontWeight.bold)), const SizedBox(width: 12), Chip(label: Text(history.last['severity'] ?? 'N/A'))]),
        const SizedBox(height: 8),
